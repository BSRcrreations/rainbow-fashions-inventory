...
export default function ProductFirstStockEntry({ session, canManageCatalog, onSaved }: Props) {
  ...
  async function stage(confirmShared = false) {
    ...
    try {
      ...
      if (cause instanceof ApiError && cause.code === "SHARED_BARCODE_CONFIRMATION_REQUIRED") {
        const details = cause.details ?? {}; setSharedConfirmation({ barcode: typeof details.barcode === "string" ? details.barcode : barcode, sizes: Array.isArray(details.existing_sizes) ? details.existing_sizes.filter((value): value is string => typeof value === "string") : [] });
  ...
  return <section className="ds-surface space-y-5 p-4 sm:p-5" data-testid="product-first-entry">
    ...
    {sharedConfirmation ? <section className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><h3 className="font-semibold">Shared barcode detected</h3><p className="mt-1">This barcode is already used for {product?.name} / {variant?.color || "this colour"}. Existing sizes: {sharedConfirmation.sizes.join(", ") || "another size"}. Add {variant?.size || "this size"} to this shared barcode group?</p><div className="mt-3 flex gap-2"><Button type="button" onClick={() => void stage(true)}>Add {variant?.size || "Variant"} & Stage</Button><Button type="button" variant="secondary" onClick={() => setSharedConfirmation(null)}>Cancel</Button></div></section> : null}
