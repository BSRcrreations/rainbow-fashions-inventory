...
export default function BarcodeLabelDialog({ open, product, onClose }: BarcodeLabelDialogProps) {
  ...
  function printLabels() {
    ...
    const brand = product.brand?.name ?? "Rainbow Fashions";
    const label = `<article class="label"><div class="shop">Rainbow Fashions</div><div class="name">${escapeHtml(product.name)}</div><div class="meta">${escapeHtml([brand, product.size, product.color].filter(Boolean).join(" · ") || "Standard")}</div><svg class="barcode" viewBox="${barcodeRef.current.getAttribute("viewBox") ?? "0 0 1 1"}" xmlns="http://www.w3.org/2000/svg">${barcodeRef.current.innerHTML}</svg><div class="code">${escapeHtml(barcode)}</div><div class="details"><span>${escapeHtml(shortDate(product.product_date))}</span><strong>${escapeHtml(money(product.selling_price))}</strong></div></article>`;
    printWindow.document.write(`<!doctype html><html><head><title>Barcode labels</title><style>@page { size: 50mm 30mm; margin: 0; } * { box-sizing: border-box; } body { margin: 0; font-family: Arial, sans-serif; } .label { width: 50mm; height: 30mm; padding: 2.2mm; overflow: hidden; page-break-after: always; color: #111827; } .shop { font-size: 6pt; font-weight: 700; color: #0f766e; letter-spacing: .12mm; text-transform: uppercase; } .name { font-size: 9pt; line-height: 1.15; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .meta { margin-top: .5mm; font-size: 6.5pt; color: #475569; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .barcode { display: block; width: 45.6mm; height: 9.2mm; margin: 1.2mm 0 .2mm; } .code { font-family: monospace; font-size: 7pt; text-align: center; letter-spacing: .35mm; } .details { display: flex; justify-content: space-between; margin-top: 1mm; font-size: 7pt; }</style></head><body>${label.repeat(copies)}</body></html>`);
