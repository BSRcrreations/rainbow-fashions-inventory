...
interface Props {
  ...
  onSaved: () => void;
  onEditProduct: () => void;
  onUseExistingVariant: (variantId: string) => void;
}

type DeleteCheck = { can_delete: boolean; reason?: string | null; references: Record<string, number> };

type ExistingVariant = { id: string; size?: string | null; color?: string | null; current_stock: number; is_active: boolean };
...
export default function VariantManagementDialog({ open, product, variant, canPermanentlyDelete, onClose, onSaved, onEditProduct, onUseExistingVariant }: Props) {
  ...
  const [existingVariant, setExistingVariant] = useState<ExistingVariant | null>(null);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (cause instanceof ApiError && cause.code === "VARIANT_ALREADY_EXISTS") {
        const existing = cause.details?.existing_variant;
        if (existing && typeof existing === "object" && "id" in existing && typeof existing.id === "string") setExistingVariant(existing as ExistingVariant);
      }
  ...
  return <Dialog open={open} title="EDIT VARIANT" description="Changes apply only to this variant. Scanning or selecting loaded these values; nothing is saved until Save Changes." onClose={onClose} maxWidth="lg">
    <form className="space-y-5" onSubmit={submit}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="field-label">Size<input className="field-input mt-1" list="variant-size-suggestions" placeholder="e.g. M, 32B, Free Size" value={size} onChange={(event) => setSize(event.target.value)} /><datalist id="variant-size-suggestions"><option value="XS" /><option value="S" /><option value="M" /><option value="L" /><option value="XL" /><option value="2XL" /><option value="3XL" /><option value="32B" /><option value="34C" /><option value="Free Size" /><option value="Custom Size" /></datalist><span className="mt-1 block text-xs text-slate-500">Type any brand-specific size, including 32B, Free Size, or Custom Size.</span></label>
      ...
      {existingVariant ? <section className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><h3 className="font-semibold">Use the existing variant</h3><p className="mt-1"><strong>{existingVariant.size || "Standard"}{existingVariant.color ? ` · ${existingVariant.color}` : ""}</strong> already exists for this product. It has {existingVariant.current_stock} in stock.</p><p className="mt-1">No variants were merged or changed.</p><div className="mt-3 flex justify-end"><Button type="button" variant="secondary" onClick={() => onUseExistingVariant(existingVariant.id)}>Use Existing {existingVariant.size || "Variant"} Variant</Button></div></section> : null}
      ...
      <section className="rounded-lg border border-teal-100 bg-teal-50/50 p-3 text-sm text-teal-950"><strong>Current stock: {variant.current_stock}</strong><p className="mt-1">Stock quantity is managed from Stock Adjustment / Stock Correction.</p></section>
