...
export default function VariantManagementDialog({ open, product, variant, canPermanentlyDelete, onClose, onSaved }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await api.patch(`/product-variants/${variant.id}`, { size: size || null, color: color || null, mrp: mrp || null, selling_price: sellingPrice, purchase_cost: purchaseCost, internal_sku: sku, barcode, scan_unit: scanUnit, pieces_per_pack: scanUnit === "PACK" ? Number(piecesPerPack) : 1 });
  ...
  return <Dialog open={open} title={`Manage variant — ${product.name}`} description="Changes apply only to this size/colour variant. Stock is managed separately." onClose={onClose} maxWidth="lg">
