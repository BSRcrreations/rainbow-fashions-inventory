...
const blank = {
  ...
  purchase_cost: "", mrp: "", selling_price: "", image_url: "",
};
...
export default function BarcodeOnboardingDialog({ open, barcode, session, onClose, onSaved }: Props) {
  const [action, setAction] = useState<OnboardingAction>("EXISTING_VARIANT");
  const [selectedVariantId, setSelectedVariantId] = useState("");
  const [selectedProductId, setSelectedProductId] = useState("");
  const [search, setSearch] = useState("");
  const [form, setForm] = useState(() => ({ ...blank, category_id: session.default_category_id ?? "", brand_id: session.default_brand_id ?? "", purchase_cost: session.default_purchase_cost ?? "", selling_price: session.default_selling_price ?? "" }));
  ...
  const catalogQuery = useQuery({ queryKey: ["stock-onboard-catalog", search], queryFn: () => api.get<SaleCatalogProduct[]>(`/sales/catalog${search.trim() ? `?search=${encodeURIComponent(search.trim())}` : ""}`), enabled: open });
  ...
  const onboardMutation = useMutation({
    mutationFn: () => {
      ...
      return api.post<StockScanSession>("/barcodes/onboard-product", {
        session_id: session.id, action, barcode, product_variant_id: action === "EXISTING_VARIANT" ? selectedVariantId : undefined,
        existing_product_id: action === "NEW_VARIANT" ? selectedProductId : undefined,
        ...form, quantity: Number(form.quantity), package_quantity: Number(form.package_quantity), purchase_cost: Number(form.purchase_cost), mrp: form.mrp ? Number(form.mrp) : null, selling_price: Number(form.selling_price),
      });
    },
  ...
  return <Dialog open={open} onClose={onClose} title="Set up unassigned barcode" description={`Barcode ${barcode} is not assigned to a product. Verify all values before saving.`} maxWidth="xl">
    ...
    <div className="mt-4 grid gap-5 lg:grid-cols-[minmax(0,1fr)_260px]">
      <div className="space-y-4">
        {action !== "NEW_PRODUCT" ? <section><label className="field-label">Find existing product or variant<div className="relative mt-1"><Search size={16} className="absolute left-3 top-3 text-muted" /><input className="field-input pl-9" placeholder="Search name, SKU, style or barcode" value={search} onChange={(event) => setSearch(event.target.value)} /></div></label><div className="mt-2 max-h-52 space-y-2 overflow-y-auto">{(catalogQuery.data ?? []).map((product) => <div key={product.product_id} className={`rounded-lg border p-3 ${selectedProductId === product.product_id ? "border-primary-400 bg-primary-50" : "border-border"}`}><button type="button" className="w-full text-left" onClick={() => { setSelectedProductId(product.product_id); setAction("NEW_VARIANT"); }}><div className="font-semibold">{product.name}</div><div className="text-xs text-muted">{product.category_name} · {product.brand_name}</div></button>{action === "EXISTING_VARIANT" ? <div className="mt-2 grid gap-1 sm:grid-cols-2">{product.variants.map((variant) => <button key={variant.variant_id} type="button" onClick={() => setSelectedVariantId(variant.variant_id)} className={`rounded-md border px-2 py-2 text-left text-xs ${selectedVariantId === variant.variant_id ? "border-primary-500 bg-primary-100" : "border-border hover:bg-slate-50"}`}><div className="font-semibold">{detail(variant)}</div><div>{variant.sku} · {variant.available_stock} available</div></button>)}</div> : null}</div>)}</div></section> : null}
        {action === "NEW_PRODUCT" ? <section className="grid gap-3 sm:grid-cols-2"><label className="field-label sm:col-span-2">Product name<input className="field-input" value={form.product_name} onChange={(event) => update("product_name", event.target.value)} /></label><label className="field-label">Category<select className="field-input" value={form.category_id} onChange={(event) => chooseCategory(event.target.value)}><option value="">Select category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label className="field-label">Subcategory<select className="field-input" value={form.subcategory_id} onChange={(event) => update("subcategory_id", event.target.value)}><option value="">Use first active subcategory</option>{subcategories.map((subcategory) => <option key={subcategory.id} value={subcategory.id}>{subcategory.name}</option>)}</select></label><div className="sm:col-span-2 flex flex-wrap items-end gap-2"><label className="field-label flex-1">Create category inline<input className="field-input" placeholder="Category name" value={newCategoryName} onChange={(event) => setNewCategoryName(event.target.value)} /></label><Button type="button" variant="secondary" disabled={!newCategoryName.trim() || createCategoryMutation.isPending} onClick={() => createCategoryMutation.mutate()}><Plus size={16} /> Add category</Button></div><label className="field-label">Brand<select className="field-input" disabled={!form.category_id} value={form.brand_id} onChange={(event) => update("brand_id", event.target.value)}><option value="">Select brand</option>{brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label><div className="flex items-end gap-2"><label className="field-label flex-1">Create brand inline<input className="field-input" disabled={!form.category_id} placeholder="Brand or Unbranded" value={newBrandName} onChange={(event) => setNewBrandName(event.target.value)} /></label><Button type="button" variant="secondary" disabled={!form.category_id || !newBrandName.trim() || createBrandMutation.isPending} onClick={() => createBrandMutation.mutate()}><Plus size={16} /></Button></div></section> : null}
        ...
        <section className="grid gap-3 sm:grid-cols-3"><label className="field-label">Quantity to add<input className="field-input" min="1" type="number" value={form.quantity} onChange={(event) => update("quantity", event.target.value)} /></label><label className="field-label">Purchase cost<input className="field-input" min="0" step="0.01" type="number" value={form.purchase_cost} onChange={(event) => update("purchase_cost", event.target.value)} /></label><label className="field-label">MRP <span className="text-muted">optional</span><input className="field-input" min="0" step="0.01" type="number" value={form.mrp} onChange={(event) => update("mrp", event.target.value)} /></label><label className="field-label">Selling price<input className="field-input" min="0" step="0.01" type="number" value={form.selling_price} onChange={(event) => update("selling_price", event.target.value)} /></label><label className="field-label">Pieces per scan<input className="field-input" min="1" type="number" value={form.package_quantity} onChange={(event) => update("package_quantity", event.target.value)} /></label><label className="field-label">Scan unit<select className="field-input" value={form.scan_unit} onChange={(event) => update("scan_unit", event.target.value)}><option value="PIECE">Piece</option><option value="PACK">Pack</option></select></label><label className="field-label sm:col-span-2">How is it sold?<select className="field-input" value={form.sale_mode} onChange={(event) => update("sale_mode", event.target.value)}><option value="PIECE_ONLY">Individual pieces only</option><option value="PACK_ONLY">Complete pack only</option><option value="BOTH">Both pack and individual pieces</option></select></label></section>
      </div>
        <aside className="space-y-3 rounded-xl border border-border bg-slate-50 p-4"><div><div className="text-sm font-semibold">Label photo</div><p className="mt-1 text-xs text-muted">OCR suggestions are optional and always require review.</p></div><label className="flex min-h-28 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 bg-white p-3 text-center text-sm text-slate-600 hover:border-primary-400"><ImagePlus size={20} />{uploadMutation.isPending ? "Reading label..." : "Upload or take label photo"}<input className="sr-only" accept="image/jpeg,image/png,image/webp" capture="environment" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) uploadMutation.mutate(file); }} /></label>{imageInfo ? <><img className="h-28 w-full rounded-lg border border-border object-contain bg-white" src={imageInfo.image_url} alt="Uploaded product label" /><div className="space-y-2">{Object.entries(imageInfo.suggestions).map(([key, item]) => <div key={key} className={`rounded-md p-2 text-xs ${item.confidence < 0.8 ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-800"}`}><div className="font-semibold capitalize">{key.replace(/_/g, " ")}</div><div>{item.value} · {Math.round(item.confidence * 100)}%</div></div>)}</div></> : null}<div className="rounded-lg border border-primary-100 bg-primary-50 p-3 text-xs text-primary-900"><strong>Conversion:</strong> {form.quantity || "0"} scans × {form.package_quantity || "0"} {form.scan_unit === "PACK" ? "pieces per pack" : "piece"} = {Number(form.quantity || 0) * Number(form.package_quantity || 0)} base pieces.</div></aside>
    </div>
    <div className="mt-6 flex justify-end gap-2 border-t border-border pt-4"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="button" disabled={onboardMutation.isPending} onClick={() => onboardMutation.mutate()}>{onboardMutation.isPending ? "Saving" : <><PackagePlus size={17} /> Save product & add to stock</>}</Button></div>
  </Dialog>;
}
