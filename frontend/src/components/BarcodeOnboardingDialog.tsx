...
export default function BarcodeOnboardingDialog({ open, barcode, session, initialAction = "NEW_PRODUCT", initialQuantity = "1", onClose, onSaved }: Props) {
  ...
  return <Dialog open={open} onClose={onClose} title="Barcode not registered" description="Create or select the exact sellable product without leaving this stock draft." maxWidth="xl">
    ...
    <div className="mt-6 flex justify-end gap-2 border-t border-border pt-4"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="button" disabled={onboardMutation.isPending || (action === "EXISTING_VARIANT" && !selectedVariantId)} onClick={() => onboardMutation.mutate()}>{onboardMutation.isPending ? "Saving" : <><PackagePlus size={17} /> {action === "EXISTING_VARIANT" ? "Assign barcode" : action === "NEW_PRODUCT" ? "Create product and add to stock draft" : "Save and add to stock draft"}</>}</Button></div>
  </Dialog>;
}
