...
export default function BarcodeOnboardingDialog({ open, barcode, session, initialQuantity = "1", onClose, onSaved }: Props) {
  const [action, setAction] = useState<OnboardingAction>("NEW_PRODUCT");
  ...
  return <Dialog open={open} onClose={onClose} title="Barcode not registered" description="Create or select the exact sellable product without leaving this stock draft." maxWidth="xl">
    ...
    <div className="mt-6 flex justify-end gap-2 border-t border-border pt-4"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="button" disabled={onboardMutation.isPending} onClick={() => onboardMutation.mutate()}>{onboardMutation.isPending ? "Saving" : <><PackagePlus size={17} /> {action === "NEW_PRODUCT" ? "Create product and add to stock draft" : "Save and add to stock draft"}</>}</Button></div>
  </Dialog>;
}
