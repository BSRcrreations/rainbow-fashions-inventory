...
import { ChevronLeft, ChevronRight, Download, FileText, Plus, Printer, Search, ShoppingCart } from "lucide-react";
    ...
    <>
      ...
      {query.isLoading ? <SkeletonRows rows={7} /> : query.error ? <ErrorState message={query.error instanceof Error ? query.error.message : "Unable to load sales history"} /> : sales.length ? (
        <div className="overflow-hidden rounded-md border border-line bg-white">
          ...
          <div className="hidden overflow-x-auto md:block"><table className="min-w-[850px] divide-y divide-line text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Invoice</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Payment</th><th className="px-4 py-3">Cashier</th><th className="px-4 py-3 text-right">Total</th><th /></tr></thead><tbody className="divide-y divide-line">{sales.map((sale) => <tr key={sale.id}><td className="px-4 py-3 font-medium">{sale.invoice_number}</td><td className="px-4 py-3 text-slate-600">{shortDate(sale.sale_date)}</td><td className="px-4 py-3">{sale.customer_name || "Walk-in"}</td><td className="px-4 py-3">{sale.payment_mode}</td><td className="px-4 py-3">{sale.cashier?.full_name || "-"}</td><td className="px-4 py-3 text-right font-semibold">{money(sale.total_amount)}</td><td className="px-4 py-3 text-right"><Button type="button" size="sm" variant="secondary" onClick={() => setSelected(sale)}>View</Button></td></tr>)}</tbody></table></div>
