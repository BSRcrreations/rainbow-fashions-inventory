...
export default function SalesHistoryPage() {
  ...
  const params = useMemo(() => { const value = new URLSearchParams({ page: String(page), page_size: String(pageSize) }); if (invoiceNumber.trim()) value.set("invoice_number", invoiceNumber.trim()); if (customerName.trim()) value.set("customer_name", customerName.trim()); if (cashierName.trim()) value.set("cashier_name", cashierName.trim()); if (paymentMode) value.set("payment_mode", paymentMode); if (statusFilter) value.set("status_filter", statusFilter); if (startDate) value.set("start_date", startDate); if (endDate) value.set("end_date", endDate); return value; }, [cashierName, customerName, endDate, invoiceNumber, page, pageSize, paymentMode, startDate, statusFilter]);
  ...
  async function beginDelete(ids: string[]) {
    try {
      ...
      const check = await api.post<{ deletable: Array<{ reference: string }>; requires_void: Array<{ reference: string }>; blocked: Array<{ reason: string }> }>("/sales/delete-check", { sale_ids: ids });
      if (check.blocked.length) { toast.error(check.blocked[0].reason); return; }
      setDeleteIds(ids); setDeleteSummary(`${check.deletable.length} will be permanently deleted and ${check.requires_void.length} will be voided with stock restoration.`);
    } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to check sale"); }
  }

  async function confirmDelete(password: string) {
    setDeleting(true);
    try {
      const result = await api.post<{ deleted: unknown[]; voided: unknown[] }>("/sales/delete", { sale_ids: deleteIds, delete_password: password }, { "Idempotency-Key": crypto.randomUUID() });
      toast.success(`${result.deleted.length} sales deleted, ${result.voided.length} voided`); setDeleteIds([]); setSelectedIds(new Set()); refresh();
    } catch (error) { const message = error instanceof Error ? error.message : "Unable to delete sale"; setDeleteError(message); setDeleteRequestId(error instanceof ApiError ? error.requestId : undefined); toast.error(message); } finally { setDeleting(false); }
  }
  ...
  return (
    <>
      ...
      <div className="mb-5 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-3">
        ...
        <select aria-label="Sale status" className="focus-ring h-10 rounded-md border border-line px-3" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setPage(1); setSelectedIds(new Set()); }}><option value="">All statuses</option>{["COMPLETED", "DRAFT", "CANCELLED", "VOIDED"].map((status) => <option key={status}>{status}</option>)}</select>
      ...
      {query.isLoading ? <SkeletonRows rows={7} /> : query.error ? <ErrorState message={query.error instanceof Error ? query.error.message : "Unable to load sales history"} /> : sales.length ? (
        <div className="overflow-hidden rounded-md border border-line bg-white">
          <div className="divide-y divide-line md:hidden">{sales.map((sale) => <div key={sale.id} className="flex items-start gap-3 p-4 hover:bg-slate-50">{user?.role === "OWNER" ? <input aria-label={`Select ${sale.invoice_number}`} className="mt-1" type="checkbox" checked={selectedIds.has(sale.id)} onChange={(event) => setSelectedIds((current) => { const next = new Set(current); if (event.target.checked) next.add(sale.id); else next.delete(sale.id); return next; })} /> : null}<button type="button" className="min-w-0 flex-1 text-left" onClick={() => setSelected(sale)}><div className="flex justify-between gap-3"><div><div className="font-semibold">{sale.invoice_number}</div><div className="mt-1 text-xs text-slate-500">{sale.customer_name || "Walk-in"} · {sale.cashier?.full_name || "-"}</div></div><div className="text-right"><div className="font-semibold">{money(sale.total_amount)}</div><div className="text-xs text-slate-500">{shortDate(sale.sale_date)}</div></div></div></button></div>)}</div>
          <div className="hidden overflow-x-auto md:block"><table className="min-w-[950px] divide-y divide-line text-sm"><thead className="sticky top-0 bg-slate-50 text-left text-xs uppercase text-slate-500"><tr>{user?.role === "OWNER" ? <th className="px-4 py-3"><input aria-label="Select all sales on page" type="checkbox" checked={sales.length > 0 && selectedIds.size === sales.length} onChange={(event) => setSelectedIds(event.target.checked ? new Set(sales.map((sale) => sale.id)) : new Set())} /></th> : null}<th className="px-4 py-3">Invoice</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Items</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Total</th><th /></tr></thead><tbody className="divide-y divide-line">{sales.map((sale) => <tr key={sale.id}>{user?.role === "OWNER" ? <td className="px-4 py-3"><input aria-label={`Select ${sale.invoice_number}`} type="checkbox" checked={selectedIds.has(sale.id)} onChange={(event) => setSelectedIds((current) => { const next = new Set(current); if (event.target.checked) next.add(sale.id); else next.delete(sale.id); return next; })} /></td> : null}<td className="px-4 py-3 font-medium"><button onClick={() => setSelected(sale)} className="text-primary hover:underline">{sale.invoice_number}</button></td><td className="px-4 py-3 text-slate-600">{shortDate(sale.sale_date)}</td><td className="px-4 py-3">{sale.customer_name || "Walk-in"}</td><td className="px-4 py-3">{sale.items.reduce((total, item) => total + item.quantity, 0)}</td><td className="px-4 py-3"><StatusBadge value={sale.status} /></td><td className="px-4 py-3 text-right font-semibold">{money(sale.total_amount)}</td><td className="px-4 py-3 text-right"><Button type="button" size="icon" variant="ghost" onClick={() => setActionSale(sale)} aria-label={`Actions for ${sale.invoice_number}`}><MoreHorizontal size={18} /></Button></td></tr>)}</tbody></table></div>
