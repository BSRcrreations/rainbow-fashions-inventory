...
import { categoryBrandLine, productCardMrpText, productCardVariantSummary } from "./newSaleCard";
import { catalogItemFromBarcode, firstSellableProduct, mergeCartVariant } from "./newSaleLogic";
...
export function ProductGroupCard({ product, selected, onChoose }: { product: SaleCatalogProduct; selected: boolean; onChoose: () => void }) {
  ...
  const mrpText = productCardMrpText(product);
  return <button type="button" aria-pressed={selected} onClick={onChoose} className={`group flex min-h-[156px] w-full items-start gap-3 rounded-xl border p-4 text-left shadow-sm transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 ${selected ? "border-teal-600 bg-teal-50/50 shadow-md" : "border-slate-200 bg-white hover:border-teal-400 hover:shadow-md"}`}>
    <ProductVisual product={product} compact />
    <div className="flex min-w-0 flex-1 flex-col self-stretch"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><h3 className="max-h-10 overflow-hidden break-normal text-base font-extrabold leading-5 tracking-tight text-slate-950 [hyphens:none] [overflow-wrap:normal]">{product.name}</h3></div>{product.brand_name ? <span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold ring-1 ring-inset ${brandBadgeTone(product.brand_name)}`}>{product.brand_name}</span> : null}</div>
      <p className="mt-1.5 truncate text-sm text-slate-500">{categoryBrand}</p>
      {variantSummary ? <p className="mt-1 truncate text-sm font-semibold text-slate-700">{product.variant_count > 1 ? `Preview: ${variantSummary}` : variantSummary}</p> : null}
      <p className="mt-1 truncate text-sm font-bold text-slate-900">{mrpText}</p>
...
function ProductGroupGrid({ products, selected, onChoose }: { products: SaleCatalogProduct[]; selected: SaleCatalogProduct | null; onChoose: (product: SaleCatalogProduct) => void }) {
  ...
  return <div className="mt-4 grid grid-cols-1 gap-3 min-[760px]:grid-cols-2">{products.map((product) => <ProductGroupCard key={product.product_id} product={product} selected={selected?.product_id === product.product_id} onChoose={() => onChoose(product)} />)}</div>;
}
  ...
  return <article className="p-4" data-testid={`cart-line-${line.variant.variant_id}`}>
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        <h3 className="truncate font-bold text-slate-950">{line.product.name}</h3>
...
function SaleTotals({ subtotal, discountType, discountValue, preview, itemCount, lineCount, pending }: { subtotal: number; discountType: SaleDiscountType; discountValue: string; preview: SaleDiscountPreview; itemCount: number; lineCount: number; pending: boolean }) {
  return <div className="border-t border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center justify-between text-xs font-semibold text-slate-500"><span>{lineCount} product line{lineCount === 1 ? "" : "s"}</span><span>{itemCount} unit{itemCount === 1 ? "" : "s"}</span></div><div className="space-y-2 text-sm"><div className="flex justify-between text-slate-600"><span>Subtotal</span><span>{money(subtotal)}</span></div><div className="flex justify-between text-slate-600"><span>{saleDiscountLabel(discountType, discountValue)}</span><span>- {money(preview.discountAmount)}</span></div><div className="flex justify-between border-t border-slate-200 pt-3 text-xl font-extrabold text-slate-950"><span>Total</span><span>{money(preview.total)}</span></div></div><Button type="submit" className="mt-4 h-12 w-full text-base" disabled={!lineCount || pending || !preview.valid}><ReceiptText size={18} /> {pending ? "Completing Sale…" : `Complete Sale · ${money(preview.total)}`}</Button></div>;
}

export function CurrentSalePanel({ cart, customerName, paymentMode, discountType, discountValue, subtotal, preview, pending, onCustomer, onPayment, onDiscountType, onDiscountValue, onChangeQuantity, onSetQuantity, onRemove, onClear, onSubmit }: { cart: CartLine[]; customerName: string; paymentMode: PaymentMode; discountType: SaleDiscountType; discountValue: string; subtotal: number; preview: SaleDiscountPreview; pending: boolean; onCustomer: (value: string) => void; onPayment: (value: PaymentMode) => void; onDiscountType: (value: SaleDiscountType) => void; onDiscountValue: (value: string) => void; onChangeQuantity: (variantId: string, delta: number) => void; onSetQuantity: (variantId: string, quantity: number) => void; onRemove: (variantId: string) => void; onClear: () => void; onSubmit: (event: FormEvent) => void }) {
  const itemCount = cart.reduce((totalItems, line) => totalItems + line.quantity, 0);
  return <form onSubmit={onSubmit} className="flex min-h-0 max-h-[calc(100dvh-6rem)] w-full flex-col overflow-y-auto overscroll-contain rounded-2xl border border-slate-200 bg-white shadow-xl">
    ...
    <div className="min-h-[148px] max-h-[40svh] shrink-0 overflow-x-visible overflow-y-auto divide-y divide-slate-100" data-testid="cart-item-list">
    ...
    <div className="shrink-0 space-y-3 border-t border-slate-100 px-4 py-3"><label className="field-label text-xs">Customer <span className="text-slate-400">Optional</span><input className="field-input mt-1 h-10" placeholder="Walk-in customer" value={customerName} onChange={(event) => onCustomer(event.target.value)} /></label><div><div className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Payment method</div><div className="grid grid-cols-4 gap-2">{paymentOptions.map((option) => { const Icon = option.icon; return <button key={option.value} type="button" onClick={() => onPayment(option.value)} className={`grid h-12 place-items-center rounded-xl border text-xs font-bold transition ${paymentMode === option.value ? "border-teal-600 bg-teal-50 text-teal-800 shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-teal-200"}`}><Icon size={17} /><span>{option.label}</span></button>; })}</div></div><div><label className="field-label text-xs" htmlFor="sale-discount-type">Discount type</label><select id="sale-discount-type" aria-label="Discount type" className="field-input mt-1 h-10" value={discountType} onChange={(event) => onDiscountType(event.target.value as SaleDiscountType)}><option value="PERCENTAGE">Percentage (%)</option><option value="FIXED_AMOUNT">Fixed amount (₹)</option></select></div><div><label className="field-label text-xs" htmlFor="sale-discount-value">Discount value</label><div className="relative mt-1"><input id="sale-discount-value" aria-label="Discount value" className="field-input h-10 pr-10" inputMode="decimal" pattern="[0-9]*[.]?[0-9]{0,2}" value={discountValue} onWheel={(event) => event.currentTarget.blur()} onChange={(event) => onDiscountValue(event.target.value)} /><span className="pointer-events-none absolute inset-y-0 right-3 grid place-items-center text-sm font-bold text-slate-500">{discountType === "PERCENTAGE" ? "%" : "₹"}</span></div>{preview.error ? <p role="alert" className="mt-1 text-xs font-medium text-rose-700">{preview.error}</p> : null}</div><div><div className="mb-1 text-xs font-bold uppercase tracking-wide text-slate-500">Quick percentage</div><div className="grid grid-cols-4 gap-2">{[5, 10, 15, 20].map((value) => <button key={value} type="button" className="h-9 rounded-lg border border-teal-200 bg-teal-50 text-xs font-bold text-teal-800 hover:border-teal-400" onClick={() => { onDiscountType("PERCENTAGE"); onDiscountValue(String(value)); }}>{value}%</button>)}</div></div><Button type="button" variant="ghost" size="sm" className="w-full" disabled={discountValue === "0"} onClick={() => onDiscountValue("0")}>Clear discount</Button></div>
    <SaleTotals subtotal={subtotal} discountType={discountType} discountValue={discountValue} preview={preview} itemCount={itemCount} lineCount={cart.length} pending={pending} />
  </form>;
}
...
export default function NewSalePage() {
  const toast = useToast(); const queryClient = useQueryClient(); const navigate = useNavigate(); const searchRef = useRef<HTMLInputElement>(null);
  ...
  const completeMutation = useMutation({ mutationFn: () => { if (!cart.length) throw new Error("Add at least one product variant to the sale"); if (!discountPreview.valid) throw new Error(discountPreview.error); return api.post<Sale>("/sales", { customer_name: customerName.trim() || null, payment_mode: paymentMode, discount_type: discountType, discount_value: discountValue || "0", items: cart.map((line) => ({ product_variant_id: line.variant.variant_id, quantity: line.quantity })) }); }, onSuccess: (sale) => { setCompletedSale(sale); setCart([]); setCustomerName(""); setDiscountType("PERCENTAGE"); setDiscountValue("0"); setPaymentMode("CASH"); setMobileCartOpen(false); toast.success(`Sale ${sale.invoice_number} completed`); for (const key of ["pos-variant-catalog", "products", "sales-history", "sales-dashboard", "stock-history"]) void queryClient.invalidateQueries({ queryKey: [key] }); }, onError: (cause) => toast.error(cause instanceof Error ? cause.message : "Unable to complete sale") });
  const panelProps = { cart, customerName, paymentMode, discountType, discountValue, subtotal, preview: discountPreview, pending: completeMutation.isPending, onCustomer: setCustomerName, onPayment: setPaymentMode, onDiscountType: setDiscountType, onDiscountValue: setDiscountValue, onChangeQuantity: changeQuantity, onSetQuantity: setQuantity, onRemove: (variantId: string) => setCart((current) => current.filter((line) => line.variant.variant_id !== variantId)), onClear: () => setClearOpen(true), onSubmit: (event: FormEvent) => { event.preventDefault(); if (!completeMutation.isPending) completeMutation.mutate(); } };
  ...
  return <>
    ...
    <div className="grid min-w-0 gap-6 pb-20 xl:grid-cols-[minmax(0,1.85fr)_minmax(360px,1fr)] xl:pb-0">
      <section className="min-w-0">
        ...
        {catalogQuery.isLoading ? <div className="mt-4"><SkeletonRows rows={6} /></div> : catalogQuery.error ? <div className="mt-4"><ErrorState message={catalogQuery.error instanceof Error ? catalogQuery.error.message : "Unable to load the sellable catalog"} /></div> : <ProductGroupGrid products={filteredProducts} selected={selectedProduct} onChoose={setSelectedProduct} />}
      </section>
      <aside className="hidden xl:block xl:sticky xl:top-20 xl:h-fit"><CurrentSalePanel {...panelProps} /></aside>
    </div>
    <MobileCartBar itemCount={itemCount} total={total} onOpen={() => setMobileCartOpen(true)} />
    <Dialog open={mobileCartOpen} title="Current Sale" description={`${cart.length} product lines · ${itemCount} units`} onClose={() => setMobileCartOpen(false)} maxWidth="lg"><CurrentSalePanel {...panelProps} /></Dialog>
    ...
    <Dialog open={Boolean(completedSale)} title="Sale completed" description={completedSale?.invoice_number} onClose={() => setCompletedSale(null)} maxWidth="md">
      {completedSale ? <div className="text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-700"><CheckCircle2 size={34} /></div><div className="mt-4 text-3xl font-bold">{money(completedSale.total_amount)}</div><p className="mt-1 text-sm text-slate-500">Payment received by {completedSale.payment_mode}</p><div className="mt-5 grid gap-2 sm:grid-cols-2"><Button type="button" variant="secondary" onClick={() => window.print()}><Printer size={16} /> Print receipt</Button><Button type="button" variant="secondary" onClick={() => { navigate(`/sales/history?invoice_number=${encodeURIComponent(completedSale.invoice_number)}`); setCompletedSale(null); }}><FileText size={16} /> View invoice</Button><Button type="button" className="sm:col-span-2" onClick={() => setCompletedSale(null)}>Start new sale</Button></div></div> : null}
    </Dialog>
  </>;
}
