...
export default function NewSalePage() {
  ...
  const [sharedBarcodeChoice, setSharedBarcodeChoice] = useState<{ barcode: string; targets: Array<{ variant_id: string; product_name: string; brand_name?: string | null; size?: string | null; color?: string | null; current_stock: number; selling_price?: number | null }> } | null>(null);
  ...
  return <>
    ...
    <Dialog open={Boolean(sharedBarcodeChoice)} onClose={() => setSharedBarcodeChoice(null)} title="Choose item" description="This manufacturer barcode is used by multiple exact variants. Choose the physical item to add.">{sharedBarcodeChoice ? <div className="space-y-2"><p className="font-mono text-sm text-slate-600">{sharedBarcodeChoice.barcode}</p>{sharedBarcodeChoice.targets.map((target) => <Button key={target.variant_id} type="button" variant="secondary" className="h-auto w-full justify-between py-3" onClick={() => { const product = productGroups.find((candidate) => candidate.variants.some((variant) => variant.variant_id === target.variant_id)); const variant = product?.variants.find((candidate) => candidate.variant_id === target.variant_id); if (!product || !variant) { setScanError("Refresh the product list and select the size again."); return; } addVariant(product, variant); setSharedBarcodeChoice(null); toast.success(`${target.product_name} / ${target.size || "Standard"} added`); }}><span><strong>{target.product_name} — {target.size || "Standard"}{target.color ? ` · ${target.color}` : ""}</strong><span className="mt-1 block text-xs text-slate-500">{target.brand_name || "Unbranded"} · {target.current_stock} in stock{target.selling_price ? ` · ${money(target.selling_price)}` : ""}</span></span></Button>)}</div> : null}</Dialog>
