...
export default function StockScanPage() {
  ...
  const scanMutation = useMutation({
    ...
    onError: (cause) => {
      ...
      if (cause instanceof ApiError && cause.code === "BARCODE_NOT_FOUND") {
        setError(""); setUnknownBarcode(latestScan); setUnknownDialogOpen(true); return;
      }
      toast.error(message);
    },
  ...
  return <>
    ...
    <Dialog open={unknownDialogOpen} onClose={() => { setUnknownDialogOpen(false); setUnknownBarcode(""); window.requestAnimationFrame(() => scannerRef.current?.focus()); }} title="Barcode available" description={`${unknownBarcode} is not currently assigned to any product.`}>
      <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-950"><div className="text-xs font-semibold uppercase tracking-wide">Barcode available</div><div className="mt-1 font-mono text-base font-bold">{unknownBarcode}</div><p className="mt-1">Nothing has been added to stock. Select the correct product and variant to assign it.</p></div>
      <div className="mt-5 flex flex-wrap justify-end gap-2"><Button type="button" variant="secondary" onClick={() => { setUnknownDialogOpen(false); window.requestAnimationFrame(() => scannerRef.current?.focus()); }}>Scan again</Button>{canAssign ? <><Button type="button" variant="secondary" onClick={() => { setEntryMode("PRODUCT_FIRST"); setUnknownDialogOpen(false); }}>Select Product</Button><Button type="button" onClick={() => { setOnboardingAction("NEW_PRODUCT"); setUnknownDialogOpen(false); setOnboardingOpen(true); }}>Create Product</Button></> : <span className="self-center text-sm text-muted">Ask an owner or manager to assign this available barcode.</span>}</div>
    </Dialog>
...
function DraftReview({ session, locked, onEdit, onQuantity, onRemove }: { session: StockScanSession; locked: boolean; onEdit: (item: StockScanSessionItem) => void; onQuantity: (itemId: string, scanned: number) => void; onRemove: (item: StockScanSessionItem) => void }) {
  return <section className="ds-surface overflow-hidden">
    ...
    <div className="divide-y divide-border">{session.items.map((item) => <article key={item.id} className="grid gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
      <div className="min-w-0"><div className="font-semibold text-foreground">{item.product_name}</div><div className="text-xs text-muted">{[item.brand_name, item.category_name].filter(Boolean).join(" · ") || "Unbranded"}</div><div className="mt-1 text-sm font-medium">{detailLabel(item)}</div><div className="mt-2 break-all font-mono text-xs text-muted">Barcode: {item.barcode}</div><div className="mt-3 grid grid-cols-3 gap-2 text-sm"><span><span className="block text-xs text-muted">Current</span><strong>{item.current_physical_stock}</strong></span><span><span className="block text-xs text-muted">Adding</span><strong>{item.base_quantity}</strong></span><span><span className="block text-xs text-muted">After</span><strong>{item.current_physical_stock + (item.difference_quantity ?? item.base_quantity)}</strong></span></div></div>
      <div className="flex flex-wrap items-end gap-2 sm:justify-end">{locked ? <span className="text-sm text-muted">Confirmed quantity: {item.scanned_quantity}</span> : <><div className="flex w-32 items-center rounded-lg border border-border bg-surface"><button type="button" className="grid h-9 w-9 place-items-center hover:bg-slate-50" onClick={() => onQuantity(item.id, Math.max(0, item.scanned_quantity - 1))} aria-label={`Decrease ${item.product_name}`}><Minus size={15} /></button><input aria-label={`Scanned quantity for ${item.product_name}`} className="h-9 w-12 border-x border-border text-center outline-none" value={item.scanned_quantity} type="number" min="0" onChange={(event) => onQuantity(item.id, Number(event.target.value) || 0)} /><button type="button" className="grid h-9 w-9 place-items-center hover:bg-slate-50" onClick={() => onQuantity(item.id, item.scanned_quantity + 1)} aria-label={`Increase ${item.product_name}`}><Plus size={15} /></button></div><Button type="button" size="sm" variant="secondary" onClick={() => onEdit(item)}>Edit</Button><Button type="button" size="sm" variant="secondary" className="text-error" onClick={() => onRemove(item)}>Remove</Button></>}</div>
    </article>)}</div>
  </section>;
}
