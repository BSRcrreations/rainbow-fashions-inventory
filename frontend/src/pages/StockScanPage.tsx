...
import type { ProductVariantBarcode, Purchase, SaleCatalogProduct, StockScanMode, StockScanQuantityMode, StockScanSession } from "../types";
...
export default function StockScanPage() {
  ...
  const [mode, setMode] = useState<StockScanMode>("PHYSICAL_COUNT");
  const [sessionId, setSessionId] = useState<string>(() => localStorage.getItem(sessionKey("PHYSICAL_COUNT")) ?? "");
  ...
  const [scanUnit, setScanUnit] = useState<"PIECE" | "PACK">("PIECE");
  ...
  const assignMutation = useMutation({
    mutationFn: (variantId: string) => api.post<ProductVariantBarcode>("/barcodes/onboard", { product_variant_id: variantId, barcode: unknownBarcode, package_quantity: Number(packageQuantity), scan_unit: scanUnit, inventory_unit: "PIECE", verified: true }),
    onSuccess: () => { setAssignOpen(false); const value = unknownBarcode; setBarcode(value); setUnknownBarcode(""); toast.success("Barcode assigned. Scanning it now."); window.setTimeout(() => submitScan(value), 0); },
    onError: (cause) => toast.error(cause instanceof Error ? cause.message : "Unable to assign barcode"),
  });
  const catalogQuery = useQuery({
    queryKey: ["stock-scan-catalog", catalogSearch],
    queryFn: () => api.get<SaleCatalogProduct[]>(`/sales/catalog${catalogSearch.trim() ? `?search=${encodeURIComponent(catalogSearch.trim())}` : ""}`),
    enabled: assignOpen,
  });
  ...
  function submitScan(value = barcode) {
    ...
    if (!normalized || !sessionId || scanMutation.isPending) return;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <section className="min-w-0 space-y-5">
          <form onSubmit={onSubmit} className="rounded-2xl border border-primary-200 bg-primary-50/60 p-4 shadow-sm sm:p-5"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-primary-700 text-white"><ScanLine size={22} /></div><div className="min-w-0 flex-1"><label className="text-sm font-semibold text-foreground" htmlFor="stock-scan-barcode">Scan barcode and press Enter</label><div className="mt-1 flex gap-2"><input id="stock-scan-barcode" ref={scannerRef} autoFocus className="min-w-0 flex-1 border-0 bg-transparent p-0 text-base outline-none placeholder:text-slate-400" placeholder="USB, Bluetooth, or manual entry" value={barcode} onChange={(event) => setBarcode(event.target.value)} onKeyDown={onScannerKeyDown} autoComplete="off" /><Button type="submit" disabled={!sessionId || scanMutation.isPending}>{scanMutation.isPending ? "Scanning" : "Add"}</Button></div><p className="mt-1 text-xs text-muted">Focus returns here after every scan. Leading zeros are preserved.</p></div></div>
            ...
            {unknownBarcode ? <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-rose-200 bg-white p-3"><p className="text-sm text-rose-700">Barcode <strong>{unknownBarcode}</strong> is not assigned to a product.</p>{canAssign ? <Button type="button" variant="secondary" size="sm" onClick={() => setAssignOpen(true)}><Barcode size={15} /> Assign to variant</Button> : <span className="text-xs text-muted">Ask an owner or manager to assign it.</span>}</div> : null}
          </form>
          <section className="ds-surface overflow-hidden">
            ...
            {sessionQuery.isLoading || createMutation.isPending ? <div className="p-6 text-sm text-muted">Preparing your persistent scan session...</div> : sessionQuery.error ? <div className="space-y-3 p-5"><ErrorState message={sessionQuery.error instanceof Error ? sessionQuery.error.message : "Unable to load stock session"} /><Button type="button" variant="secondary" onClick={() => { localStorage.removeItem(sessionKey(mode)); setSessionId(""); }}>Start a new session</Button></div> : session?.items.length ? <div className="overflow-x-auto"><table className="min-w-[1260px] w-full text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Product / variant</th><th className="px-4 py-3">SKU / barcode</th><th className="px-4 py-3">MRP</th><th className="px-4 py-3">Current stock</th><th className="px-4 py-3">Scans</th><th className="px-4 py-3">Pack</th><th className="px-4 py-3">Base pieces</th><th className="px-4 py-3">Difference</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y divide-border">{session.items.map((item) => <tr key={item.id} className="hover:bg-primary-50/30"><td className="px-4 py-3"><div className="font-semibold text-foreground">{item.product_name}</div><div className="text-xs text-muted">{[item.category_name, item.brand_name, detailLabel(item)].filter(Boolean).join(" · ")}</div></td><td className="px-4 py-3"><div>{item.sku}</div><div className="text-xs text-muted">{item.barcode}</div></td><td className="px-4 py-3">{money(item.mrp ?? item.selling_price)}</td><td className="px-4 py-3 font-medium"><div>{item.current_physical_stock} pieces</div><div className="text-xs text-muted">System count: {item.expected_quantity ?? "-"}</div></td><td className="px-4 py-3"><div className="flex w-32 items-center rounded-lg border border-border bg-surface"><button type="button" className="grid h-9 w-9 place-items-center hover:bg-slate-50" onClick={() => itemMutation.mutate({ itemId: item.id, scanned: Math.max(0, item.scanned_quantity - 1) })} title="Decrease quantity"><Minus size={15} /></button><input aria-label={`Scanned quantity for ${item.product_name}`} className="h-9 w-12 border-x border-border text-center outline-none" value={item.scanned_quantity} type="number" min="0" onChange={(event) => itemMutation.mutate({ itemId: item.id, scanned: Number(event.target.value) || 0 })} /><button type="button" className="grid h-9 w-9 place-items-center hover:bg-slate-50" onClick={() => itemMutation.mutate({ itemId: item.id, scanned: item.scanned_quantity + 1 })} title="Increase quantity"><Plus size={15} /></button></div></td><td className="px-4 py-3"><div>{item.package_quantity} {item.package_quantity === 1 ? "piece" : "pieces"}</div><div className="text-xs text-muted">per scan</div></td><td className="px-4 py-3 font-semibold text-primary-800">{item.base_quantity} pieces</td><td className={`px-4 py-3 font-bold ${(item.difference_quantity ?? 0) > 0 ? "text-success" : (item.difference_quantity ?? 0) < 0 ? "text-error" : "text-slate-600"}`}>{item.difference_quantity === null || item.difference_quantity === undefined ? "-" : `${item.difference_quantity > 0 ? "+" : ""}${item.difference_quantity}`}</td><td className="px-4 py-3"><button type="button" className="focus-ring grid h-9 w-9 place-items-center rounded-lg text-error hover:bg-rose-50" onClick={() => removeMutation.mutate(item.id)} title="Remove line" aria-label={`Remove ${item.product_name}`}><Trash2 size={17} /></button></td></tr>)}</tbody></table></div> : <EmptyState icon={ClipboardCheck} title="Ready to count" description="Scan a barcode or type one above. Repeated scans increase one barcode-package review row." />}
          </section>
        </section>
        <aside className="space-y-5 xl:sticky xl:top-20 xl:h-fit"><section className="ds-surface p-5"><h2 className="text-lg font-semibold">Count summary</h2><div className="mt-4 grid grid-cols-2 gap-3"><Summary label="Unique variants" value={totals.variants} /><Summary label="Barcode scans" value={totals.scans} /><Summary label="Base pieces" value={totals.pieces} tone="success" /><Summary label="Negative differences" value={totals.negative} tone="error" /></div><div className="mt-5 space-y-3 border-t border-border pt-4"><label className="field-label">Location<input className="field-input" defaultValue={session?.location_name ?? "Main store"} onBlur={(event) => { if (event.target.value.trim() && event.target.value !== session?.location_name) void api.patch<StockScanSession>(`/stock-scan/sessions/${sessionId}`, { location_name: event.target.value.trim() }).then((next) => queryClient.setQueryData(["stock-scan-session", next.id], next)).catch((cause: unknown) => toast.error(cause instanceof Error ? cause.message : "Unable to update location")); }} /></label><label className="field-label">Reference <span className="!ml-1 !text-xs !font-normal !text-slate-400">Optional</span><input className="field-input" defaultValue={session?.reference ?? ""} placeholder="Count sheet or opening reference" onBlur={(event) => { if (event.target.value !== (session?.reference ?? "")) void api.patch<StockScanSession>(`/stock-scan/sessions/${sessionId}`, { reference: event.target.value.trim() || null }).then((next) => queryClient.setQueryData(["stock-scan-session", next.id], next)).catch((cause: unknown) => toast.error(cause instanceof Error ? cause.message : "Unable to update reference")); }} /></label></div><Button type="button" className="mt-5 w-full" disabled={!session?.items.length || session.status === "CONFIRMED" || confirmMutation.isPending || mode === "STOCK_TRANSFER"} onClick={() => setConfirmOpen(true)}><PackagePlus size={18} /> Review and confirm stock</Button>{session?.status === "CONFIRMED" ? <p className="mt-3 text-sm font-medium text-success">Confirmed once. This session is locked and remains available as an audit record.</p> : null}</section><section className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600"><div className="flex items-center gap-2 font-semibold text-slate-800"><Pencil size={16} /> {modeLabel(mode)}</div><p className="mt-2">{mode === "PHYSICAL_COUNT" ? "Confirmation creates count-in or count-out ledger movements. It never overwrites stock." : mode === "OPENING_STOCK" ? "Confirmation creates an OPENING STOCK movement and a cost lot for each added exact variant." : mode === "PURCHASE_RECEIVING" ? "Confirmation records accepted receipt quantities; purchase confirmation remains the only supplier-stock entry point." : "Review the scanned draft before confirmation."}</p></section></aside>
      </div>
    </div>
