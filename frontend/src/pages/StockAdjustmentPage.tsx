...
import { draftKey, readDraft, prepareSubmission, removeDraft } from "../utils/draftStorage";
...
import type { Product, ProductVariant, SaleCatalogProduct, SaleCatalogVariant, StockHistory } from "../types";
...
export default function StockAdjustmentPage() {
  ...
  const { user } = useAuth();
  const requestKey = draftKey(user, "stock-adjustment-request");
  const [pending, setPending] = useState(() => Boolean(readDraft(requestKey, null)));
  ...
  const [remoteSelection, setRemoteSelection] = useState<VariantRow | null>(null);
  const selected = variants.find((row) => row.variant.id === variantId) ?? (remoteSelection?.variant.id === variantId ? remoteSelection : null);
  ...
  async function chooseAdjustment(_product: SaleCatalogProduct, variant: SaleCatalogVariant) {
    try {
      const product = await api.get<Product>(`/products/${variant.product_id}`);
      const exact = product.variants.find(item => item.id === variant.variant_id);
      if (!exact) throw new Error("This size is no longer available. Refresh and choose it again.");
      setRemoteSelection({ product, variant: exact });
      setVariantId(variant.variant_id); setQuantity(String(variant.scan_unit === "PACK" ? variant.pieces_per_pack || 1 : 1)); setError("");
      toast.success(`${product.name} ${variant.size || "Standard"} selected`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not select this size."); }
  }

  const mutation = useMutation({
    mutationFn: () => {
      const existing = readDraft<{key:string;payload:unknown} | null>(requestKey,null);
      if (existing) return api.post<StockHistory>("/stock/adjustments",existing.payload,{"Idempotency-Key":existing.key});
      ...
      const submission = prepareSubmission(requestKey, { product_id: selected.product.id, product_variant_id: selected.variant.id, adjustment_type: adjustmentType, reason: "MANUAL_ADJUSTMENT", direction, qty: adjustedQty, reference: reference.trim() });
      setPending(true);
      return api.post<StockHistory>("/stock/adjustments",submission.payload,{"Idempotency-Key":submission.key});
    },
    onSuccess: () => {
      removeDraft(requestKey); setPending(false); setConfirmOpen(false);
