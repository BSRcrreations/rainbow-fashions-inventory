...
from app.schemas.purchase import PurchaseFromDocumentCreate, PurchaseRead, PurchaseReviewUpdate, PurchaseUploadResponse
...
def get_purchase(purchase_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return PurchaseService(db).get(purchase_id, current_user)
