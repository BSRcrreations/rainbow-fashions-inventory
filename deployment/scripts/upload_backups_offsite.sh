...
exec 9>"$BACKUP_LOCAL_PATH/.offsite-backup.lock"
flock -n 9 || { echo "An offsite upload is already running" >&2; exit 1; }
...
backup_json=""
for attempt in $(seq 1 "$BACKUP_REMOTE_RETRY_ATTEMPTS"); do
  if candidate="$(restic backup --json --tag rainbow-fashions --tag database --tag uploads "$BACKUP_LOCAL_PATH/database" "$BACKUP_LOCAL_PATH/uploads")"; then backup_json="$candidate"; break; fi
  log "remote upload attempt ${attempt}/${BACKUP_REMOTE_RETRY_ATTEMPTS} failed"
  (( attempt < BACKUP_REMOTE_RETRY_ATTEMPTS )) && sleep $((attempt * 10))
done
[[ -n "$backup_json" ]] || { log "All remote upload attempts failed; local backups were retained."; exit 1; }
SNAPSHOT="$(printf '%s\n' "$backup_json" | sed -n 's/.*"snapshot_id":"\([^"]*\)".*/\1/p' | tail -n 1)"
[[ -n "$SNAPSHOT" ]] || { log "restic did not report a snapshot"; exit 1; }
# Restic lists the completed remote snapshot, providing post-upload verification
# without exposing remote access credentials or relying on local file names.
restic snapshots --json "$SNAPSHOT" | grep -q "\"id\":\"${SNAPSHOT}\""
