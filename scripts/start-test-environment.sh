...
case "$mode" in
  docker)
    ...
    test_compose build backend frontend
    test_compose up -d postgres
    printf 'Waiting for isolated UAT database readiness...\n'
    for _ in $(seq 1 30); do
      if test_compose exec -T postgres pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB" >/dev/null 2>&1; then
        break
      fi
      sleep 2
    done
    test_compose exec -T postgres pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB" >/dev/null
    test_compose run --rm --no-deps -e RUN_MIGRATIONS_ON_STARTUP=false backend python scripts/bootstrap_test_database.py
    test_compose run --rm --no-deps -e RUN_MIGRATIONS_ON_STARTUP=false backend alembic upgrade heads
    test_compose run --rm --no-deps -e RUN_MIGRATIONS_ON_STARTUP=false backend python scripts/seed_test_data.py
    test_compose up -d backend frontend
